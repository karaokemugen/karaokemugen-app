...
(function (yourcode) {
	...
	$(function () {
		...
		$('#choixPseudo').keypress(function (e) {
			if (e.which == 13) {
				$(this).blur();
			}
		});
					...
					.done(function (response) {
						setTimeout(function() {
							displayMessage('info', i18n.__('KARA_SUGGESTION_INFO'),
								i18n.__('KARA_SUGGESTION_LINK', response.issueURL, 'console'), '30000');
						}, 200);
					})
			}, search);
    ...
    */
	...
	window.initApp = function() {
		...
		window.buildPlaylist(scope, 1);
		window.buildPlaylist(scope, 2);
