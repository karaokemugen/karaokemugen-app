...
function Login(props: RouteComponentProps) {
	...
	const [serv, setServ] = useState(context.globalState.settings.data.config.Online.Users
		...
		try {
			...
			if (isAdminPath && role !== 'admin') {
				if (!username) {
					...
					callModal(context.globalDispatch, 'prompt', i18next.t('MAKE_ACCOUNT_ADMIN'), i18next.t('MAKE_ACCOUNT_ADMIN_MESSAGE'), async (securityCodeString: string) => {
		...
		<div className="loginContainer">
			...
			<div className="loginBox">
				...
				{activeView !== 'welcome' ? <>
					...
					<form onSubmit={onSubmit}>
						<div className="spacedSwitch">
							...
							<Switch handleChange={() => setOnlineSwitch(!onlineSwitch)}
								isChecked={onlineSwitch} />
						</div>
						<div className="loginForm">
							<label className="loginLabel">{i18next.t('USERNAME')}{onlineSwitch ?
								` @ ${i18next.t('INSTANCE_NAME_SHORT')}` : ''}</label>
							<div className="loginLine">
								<input type="text" className={`${errorBackground} ${onlineSwitch ? 'loginName' : ''}`}
									defaultValue={login} placeholder={i18next.t('USERNAME')} autoComplete="username"
									required autoFocus onChange={(event) => setLogin(event.target.value)} />
								{onlineSwitch ? <>
									<div className="arobase">@</div>
									<input type="text" className="instanceName" defaultValue={serv}
										placeholder={i18next.t('INSTANCE_NAME_SHORT')} autoComplete="off"
										onChange={(event) => setServ(event.target.value)} />
								</> : null}
							</div>
							<label className="loginLabel">{forgotPassword && !onlineSwitch ?
								i18next.t('NEW_PASSWORD') : i18next.t('PASSWORD')}
							</label>
							<div className="loginLine">
								<input type="password" className={redBorders}
									autoComplete={activeView === 'signup' ? 'new-password' : 'current-password'}
									defaultValue={password} required placeholder={i18next.t('PASSWORD')}
							...
							{activeView === 'signup' ?
								<>
									...
									<div className="loginLine">
										<input type="password" className={redBorders} required defaultValue={passwordConfirmation}
											onChange={(event) => setPasswordConfirmation(event.target.value)}
											placeholder={i18next.t('PASSWORDCONF')} autoComplete="new-password" />
									</div>
								</> : null
							}
							{isAdminPath && !isElectron() &&
								...
								<>
									...
									<div className="loginLine">
										<input type="text" placeholder={i18next.t('SECURITY_CODE')}
											defaultValue={securityCode} required autoFocus
				...
				<div className="versionKM">
					...
					<div>{`${i18next.t('VERSION')} ${context.globalState.settings.data.version.number} - ${context.globalState.settings.data.version.name}`}</div>
				</div>
			</div>
		</div>
	);
