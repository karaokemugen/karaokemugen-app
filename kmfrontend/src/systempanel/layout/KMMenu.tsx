...
function KMMenu() {
	...
	const haveMaintainerModeRepo = context.globalState.settings.data.config?.System?.Repositories
		.filter(repo => repo.Online && repo.MaintainerMode && repo.Enabled).length > 0;
	...
	return (
		...
		>
			<Menu.Item key='home'><Link to='/system/home'>{i18next.t('MENU.HOME')}</Link></Menu.Item>
			<Menu.SubMenu key="system-dropdown" title={i18next.t('MENU.SYSTEM')}>
				<Menu.Item key='log'><Link to='/system/log'>{i18next.t('MENU.LOGS')}</Link></Menu.Item>
				<Menu.Item key='options'><Link to='/system/options'>{i18next.t('MENU.ADVANCED_OPTIONS')}</Link></Menu.Item>
				<Menu.Item key='config'><Link to='/system/config'>{i18next.t('MENU.CONFIGURATION')}</Link></Menu.Item>
			...
			{haveMaintainerModeRepo ?
				<Menu.SubMenu key="maintainer-dropdown" title={i18next.t('MENU.MAINTAINER')}>
					...
					<Menu.Item key='inbox'><Link to='/system/inbox'>{i18next.t('MENU.INBOX')}</Link></Menu.Item>
				</Menu.SubMenu>: null
			}
			<Menu.SubMenu key="kara-dropdown" title={i18next.t('MENU.KARAS')}>
				<Menu.Item key='karalist'><Link to='/system/karas'>{i18next.t('MENU.LIST')}</Link></Menu.Item>
				...
				<Menu.Item key='karabatchedit'><Link to='/system/karas/batch'>{i18next.t('MENU.BATCH_EDIT')}</Link></Menu.Item>
			</Menu.SubMenu>
			<Menu.SubMenu key="tags-dropdown" title={i18next.t('MENU.TAGS')}>
				<Menu.Item key='tagsmanage'><Link to='/system/tags'>{i18next.t('MENU.LIST')}</Link></Menu.Item>
				<Menu.Item key='tagsnew'><Link to='/system/tags/new'>{i18next.t('MENU.NEW')}</Link></Menu.Item>
				<Menu.Item key='tagsduplicate'><Link to='/system/tags/duplicate'>{i18next.t('MENU.DUPLICATE')}</Link></Menu.Item>
			</Menu.SubMenu>
			<Menu.Item className="menuItemInactive" key='user'>
				<span><UserOutlined /> {context.globalState.auth.data.username}</span>
			</Menu.Item>
			...
			<Menu.Item key="logout">
				<Button icon={<LogoutOutlined />} onClick={() => logout(context.globalDispatch)}>{i18next.t('LOGOUT')}</Button>
			</Menu.Item>
		</Menu>
	);
}
