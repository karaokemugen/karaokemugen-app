		...
		return (
			<Form
				...
				initialValues={{
					...
					flag_tutorial_done: this.props.user.flag_tutorial_done,
				}}
			>
				...
				<Form.Item label={i18next.t('USERS.NICKNAME')} name="nickname" required={true}>
					<Input disabled={this.props.user.login && this.props.user.login.includes('@')} />
				</Form.Item>
				<Form.Item label={i18next.t('USERS.BIO')} required={true} name="bio">
					<Input.TextArea disabled={this.props.user.login && this.props.user.login.includes('@')} rows={2} />
				</Form.Item>
				<Form.Item label={i18next.t('USERS.EMAIL')} rules={[{ type: 'email' }]} name="email">
					<Input disabled={this.props.user.login && this.props.user.login.includes('@')} />
				</Form.Item>
				<Form.Item label={i18next.t('USERS.URL')} rules={[{ type: 'url' }]} name="url">
					<Input disabled={this.props.user.login && this.props.user.login.includes('@')} />
				</Form.Item>
				<Form.Item>
