...
export default function Inbox() {
	...
	const repoList = context.globalState.settings.data.config?.System?.Repositories
		.filter(repo => repo.Online && repo.MaintainerMode && repo.Enabled
			&& context.globalState.auth.data.onlineToken && repo.Name === context.globalState.auth.data.username.split('@')[1]);
	...
	const columns = [{
		...
		key: 'gitlab_issue',
		render: text => <a href={text}>{i18next.t('INBOX.ISSUE', {number: text.split('/')[text.split('/').length -1]})}</a>
	}, {
