...
class SuggestionModal extends Component<unknown, IState> {
	...
	async componentDidMount() {
		const response = await commandBackend('getTags', { type: 3 });
		const songtypes = response.content.map((tag: DBKaraTag) => getTagInLocale(tag));
	...
	confirmModal = async () => {
		if (this.state.name && this.state.serie && this.state.songtype) {
			...
			const response = await commandBackend('suggestKara',
			...
			displayMessage('info', <div><label>{i18next.t('KARA_SUGGESTION_INFO')}</label> <br />
				{i18next.t('KARA_SUGGESTION_LINK')} <a href={response.url}>{i18next.t('KARA_SUGGESTION_LINK_LIST')}</a>
			</div>, 30000);
