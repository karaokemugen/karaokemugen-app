	...
	return (
		<>
			{props.scope === 'admin'
				...
				<button title={i18next.t(props.isHeader ? 'TOOLTIP_REFUSE_SELECT_KARA' : 'TOOLTIP_REFUSE_KARA')}
			...
			{playlist?.plaid !== nonStandardPlaylists.favorites
				&& ((props.scope === 'admin' && playlist?.plaid !== nonStandardPlaylists.library
					&& !(props.isHeader && playlist?.flag_smart)
					&& !(playlist?.plaid === context.globalState.settings.data.state.publicPlaid
						&& oppositePlaylist?.plaid === context.globalState.settings.data.state.currentPlaid))
					|| (props.scope !== 'admin' && !props.kara?.flag_dejavu && !props.kara?.flag_playing
						&& (props.kara?.my_public_plc_id && props.kara?.my_public_plc_id[0]
							|| (playlist?.plaid === context.globalState.settings.data.state.publicPlaid
								&& props.kara.username === context.globalState.auth.data.username)))) ?
				<button title={i18next.t(props.isHeader ? 'TOOLTIP_DELETE_SELECT_KARA' : 'TOOLTIP_DELETEKARA')}
					disabled={props?.checkedKaras === 0}
					className={classValue} onClick={props.deleteKara}>
					<i className="fas fa-eraser" />
				</button> : null
			}

			{playlist?.plaid === nonStandardPlaylists.favorites ?
				<button title={i18next.t(props.isHeader ? 'TOOLTIP_DELETE_SELECT_FAVS' : 'TOOLTIP_DELETE_FAVS')}
					className={classValue + ' yellow'} onClick={props.deleteFavorite}>
					<i className="fas fa-star" />
				</button> : null
			}

			{(props.scope === 'admin' && oppositePlaylist?.plaid !== nonStandardPlaylists.library
				...
				<button
					title={props.isHeader ? i18next.t('TOOLTIP_ADD_SELECT_KARA') :
						`${i18next.t('TOOLTIP_ADDKARA')}${(props.scope === 'admin' ? ' - ' + i18next.t('TOOLTIP_ADDKARA_ADMIN') : '')}`}
					className={classValue} onContextMenu={onRightClickAdd}
			...
			{props.scope !== 'admin' && ((props.kara.public_plc_id?.length > 0
				&& props.kara.my_public_plc_id?.length === 0) || props.kara?.upvotes > 0) ?
				<button
