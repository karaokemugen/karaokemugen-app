	...
	return <div className="limited-width justified">
		...
		<ul>
			...
			<li>
				...
				<a href="#stats" onClick={() => setStats(!stats)}>{i18next.t('PRIVACY_POLICY.MORE')}</a>
				{stats ?
					<>
						<br/><p>{i18next.t('PRIVACY_POLICY.L4_DETAILS')}</p>
					</>:null}
			</li>
			<li>
				{i18next.t('PRIVACY_POLICY.L5')}&nbsp;
				<a href="#sentry" onClick={() => setSentry(!sentry)}>{i18next.t('PRIVACY_POLICY.MORE')}</a>
				{sentry ?
					<>
						<br/><p>{i18next.t('PRIVACY_POLICY.L5_DETAILS')}</p>
					</>:null}
			</li>
		</ul>
