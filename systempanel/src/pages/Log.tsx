...
import {Layout, Timeline, Collapse, Select} from 'antd';
...
interface LogState {
	log: {level:string, message:string, timestamp:string, service: string, obj?: object}[],
	error: string,
	level: string
}
...
class Log extends Component<{}, LogState> {
	...
	refresh = async () => {
		let res = await Axios.get(`/log/${this.state.level}`)
