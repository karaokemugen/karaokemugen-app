...
class Players {
	...
	private async loadBackground() {
		...
		if (conf.Player.Background) {
			...
			if (!(await asyncExists(backgroundImageFile))) {
				...
				logger.warn(`Unable to find background file ${backgroundImageFile}, reverting to default one`, {
					service: 'Player',
				});
			} else {
			...
			if (backgroundFiles.length === 0)
				backgroundFiles = await Players.extractAllBackgroundFiles([resolvedPathBundledBackgrounds()]);
		}
		...
		logger.debug(
			`Background selected : ${backgroundImageFile}${backgroundMusicFile ? ` (${backgroundMusicFile})` : ''}`,
			{ service: 'Player' }
		);
		try {
			playerState.mediaType = 'background';
			...
			if (backgroundMusicFile) {
				...
				await this.exec({
					command: ['loadfile', backgroundImageFile, 'replace', { 'force-media-title': 'Background' }],
				});
			}
		} catch (err) {
	...
	async displaySongInfo(infos: string, duration = -1, nextSong = false, spoilerAlert = false) {
		try {
			const spoilerString = spoilerAlert
