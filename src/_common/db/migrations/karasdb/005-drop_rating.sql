...
INSERT INTO tmp SELECT pk_id_kara,kid,title,NORM_title,year,songorder,videofile,videolength,gain,created_at,modified_at,viewcount,checksum FROM kara;
...
CREATE VIEW IF NOT EXISTS all_karas AS SELECT k.pk_id_kara AS kara_id, k.kid, k.title, k.NORM_title, k.videolength, k.gain, k.year, k.videofile, k.created_at, k.modified_at, k.viewcount ,k.songorder
...
INSERT INTO tmp SELECT pk_id_kara,kid,title,NORM_title,year,songorder,videofile,videolength,gain,created_at,modified_at,0,viewcount,checksum FROM kara;
PRAGMA foreign_keys = "0";
DROP TABLE kara;
ALTER TABLE tmp RENAME TO kara;
PRAGMA foreign_keys;
...
CREATE VIEW IF NOT EXISTS all_karas AS SELECT k.pk_id_kara AS kara_id, k.kid, k.title, k.NORM_title, k.videolength, k.gain, k.year, k.videofile, k.created_at, k.modified_at, k.viewcount, k.rating, k.songorder
,(select GROUP_CONCAT( s.name)
    FROM kara_serie ks
    INNER JOIN serie s ON ks.fk_id_serie = s.pk_id_serie
    WHERE k.pk_id_kara = ks.fk_id_kara
