...
VALUES(
	$user_id,
	$kara_id,
	(SELECT kid FROM karasdb.all_karas WHERE kara_id = $kara_id),
	$requested_at,
	$started_at
)
`;
...
SELECT ak.title AS title,
    ...
    (
		SELECT
			COUNT(pk_id_viewcount) AS viewcount
			FROM viewcount
...
export const getKaraViewcounts = `
SELECT ak.title AS title,
	ak.songorder AS songorder,
	ak.serie AS serie,
	ak.singer AS singer,
...
export const getAllKaras = (filterClauses, lang, orderClauses, typeClauses) => `
SELECT
...
ORDER BY ${orderClauses}
	language,
	serie IS NULL,
	serie COLLATE NOCASE,
	songtype DESC,
	songorder,
	singer COLLATE NOCASE,
	title COLLATE NOCASE
`;

export const getKaraByKID = `
SELECT ak.kara_id AS kara_id,
	...
	(CASE WHEN $dejavu_time < (
		...
		FROM viewcount WHERE fk_id_kara = ak.kara_id
		)
	    THEN 1
        ELSE 0
    END) AS flag_dejavu,
...
SELECT ak.kara_id AS kara_id,
	...
	COALESCE(
		...
		(SELECT sl.NORM_name
			FROM serie_lang sl, kara_serie ks
			WHERE sl.fk_id_serie = ks.fk_id_serie
				AND ks.fk_id_kara = kara_id AND sl.lang = ${lang.fallback}
		),
		ak.NORM_serie
	) AS NORM_serie,
    ak.serie_altname AS serie_altname,
