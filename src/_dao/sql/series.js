...
SELECT s.pk_id_serie AS serie_id,
	...
	(SELECT JSON_GROUP_OBJECT(lang,name)
		FROM serie_lang
		WHERE fk_id_serie = s.pk_id_serie
	) AS i18n,
	(SELECT GROUP_CONCAT(NORM_name)
		FROM serie_lang
		WHERE fk_id_serie = s.pk_id_serie
	) AS NORM_i18n_name,
	sid
...
UPDATE karasdb.serie
SET
	name = $name,
	NORM_name = $NORM_name,
	altname = $altname,
	NORM_altname = $NORM_altname,
	seriefile = $seriefile
WHERE pk_id_serie = $serie_id;
`;

export const deleteSeriesByKara = `
DELETE FROM karasdb.kara_serie
WHERE fk_id_kara = $kara_id
`;

export const insertKaraSeries = `
INSERT INTO karasdb.kara_serie(fk_id_kara,fk_id_serie)
