...
ALTER TABLE kara ADD COLUMN mediasize INTEGER;
...
INSERT INTO tmp SELECT pk_id_kara,kid,title,NORM_title,year,songorder,mediafile,karafile,subfile,duration,gain,created_at,modified_at FROM kara;
...
DROP VIEW all_karas;
CREATE VIEW IF NOT EXISTS all_karas AS SELECT k.pk_id_kara AS kara_id, k.kid, k.title, k.NORM_title, k.duration, k.gain, k.year, k.mediafile, k.subfile, k.created_at, k.modified_at, k.songorder, k.karafile
,(select  json_group_array(json_object('lang',sl.lang, 'name', sl.name))
    from serie_lang sl, kara_serie ks
    where ks.fk_id_serie = sl.fk_id_serie
    and k.pk_id_kara = ks.fk_id_kara
