...
export async function PostSuggestionToKaraBase(karaName: string, username: string): Promise<string> {
	...
	let title = conf.Gitlab.IssueTemplate.Suggestion.Title ? conf.Gitlab.IssueTemplate.Suggestion.Title : 'New kara suggestion: $kara';
	title = title.replace('$kara', karaName);
	let desc = conf.Gitlab.IssueTemplate.Suggestion.Description ? conf.Gitlab.IssueTemplate.Suggestion.Description : '';
