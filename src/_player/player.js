...
export async function play(mediadata) {
	...
	try {
		...
		if (mediaFile.endsWith('.mp3')) {
			...
			if (mediadata.avatar) options.push(`lavfi-complex=movie=\\'${mediadata.avatar.replace(/\\/g,'/')}\\'[logo];[logo][vid1]scale2ref=w=(ih*.128):h=(ih*.128)[logo1][base];[base][logo1]overlay=x='if(between(t,0,8)+between(t,100,108),W-(W*29/300),NAN)':y=H-(H*29/200)[vo]`);
		}
