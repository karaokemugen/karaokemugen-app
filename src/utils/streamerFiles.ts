...
async function writeRequester() {
	await fs.writeFile(
		resolve(resolvedPathStreamFiles(), 'requester.txt'),
...
async function writeKarasInPublicPL() {
	...
	await fs.writeFile(resolve(resolvedPathStreamFiles(), 'public_kara_count.txt'), karacount.toString(), 'utf-8');
}

async function writeKarasInCurrentPL() {
	const { karacount } = await getPlaylistInfo(getState().currentPlaid);
	await fs.writeFile(resolve(resolvedPathStreamFiles(), 'current_kara_count.txt'), karacount.toString(), 'utf-8');
}
