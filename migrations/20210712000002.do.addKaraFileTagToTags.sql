...
SELECT k.*,
  CASE WHEN MIN(tauthor.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tauthor.pk_tid,
	  'short', tauthor.short,
	  'name', tauthor.name,
	  'problematic', tauthor.problematic,
	  'aliases', tauthor.aliases,
	  'i18n', tauthor.i18n,
	  'priority', tauthor.priority,
	  'karafile_tag', tauthor.karafile_tag
	)::jsonb) END as authors,
  CASE WHEN MIN(tcreator.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tcreator.pk_tid,
	  'short', tcreator.short,
	  'name', tcreator.name,
	  'problematic', tcreator.problematic,
	  'aliases', tcreator.aliases,
	  'i18n', tcreator.i18n,
	  'priority', tcreator.priority,
	  'karafile_tag', tcreator.karafile_tag
	)::jsonb) END as creators,
  CASE WHEN MIN(tfamily.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tfamily.pk_tid,
	  'short', tfamily.short,
	  'name', tfamily.name,
	  'problematic', tfamily.problematic,
	  'aliases', tfamily.aliases,
	  'i18n', tfamily.i18n,
	  'priority', tfamily.priority,
	  'karafile_tag', tfamily.karafile_tag
	)::jsonb) END as families,
  CASE WHEN MIN(tgenre.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tgenre.pk_tid,
	  'short', tgenre.short,
	  'name', tgenre.name,
	  'problematic', tgenre.problematic,
	  'aliases', tgenre.aliases,
	  'i18n', tgenre.i18n,
	  'priority', tgenre.priority,
	  'karafile_tag', tgenre.karafile_tag
	)::jsonb) END as genres,
  CASE WHEN MIN(tgroup.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tgroup.pk_tid,
	  'short', tgroup.short,
	  'name', tgroup.name,
	  'problematic', tgroup.problematic,
	  'aliases', tgroup.aliases,
	  'i18n', tgroup.i18n,
	  'priority', tgroup.priority,
	  'karafile_tag', tgroup.karafile_tag
	)::jsonb) END as groups,
  CASE WHEN MIN(tlang.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tlang.pk_tid,
	  'short', tlang.short,
	  'name', tlang.name,
	  'problematic', tlang.problematic,
	  'aliases', tlang.aliases,
	  'i18n', tlang.i18n,
	  'priority', tlang.priority,
	  'karafile_tag', tlang.karafile_tag
	)::jsonb) END as languages,
  CASE WHEN MIN(tmisc.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tmisc.pk_tid,
	  'short', tmisc.short,
	  'name', tmisc.name,
	  'problematic', tmisc.problematic,
	  'aliases', tmisc.aliases,
	  'i18n', tmisc.i18n,
	  'priority', tmisc.priority,
	  'karafile_tag', tmisc.karafile_tag
	)::jsonb) END as misc,
  CASE WHEN MIN(torigin.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', torigin.pk_tid,
	  'short', torigin.short,
	  'name', torigin.name,
	  'problematic', torigin.problematic,
	  'aliases', torigin.aliases,
	  'i18n', torigin.i18n,
	  'priority', torigin.priority,
	  'karafile_tag', torigin.karafile_tag
	)::jsonb) END as origins,
  CASE WHEN MIN(tplatform.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tplatform.pk_tid,
	  'short', tplatform.short,
	  'name', tplatform.name,
	  'problematic', tplatform.problematic,
	  'aliases', tplatform.aliases,
	  'i18n', tplatform.i18n,
	  'priority', tplatform.priority,
	  'karafile_tag', tplatform.karafile_tag
	)::jsonb) END as platforms,
  CASE WHEN MIN(tserie.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tserie.pk_tid,
	  'short', tserie.short,
	  'name', tserie.name,
	  'problematic', tserie.problematic,
	  'aliases', tserie.aliases,
	  'i18n', tserie.i18n,
	  'priority', tserie.priority,
	  'karafile_tag', tserie.karafile_tag
	)::jsonb) END as series,
  CASE WHEN MIN(tsinger.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tsinger.pk_tid,
	  'short', tsinger.short,
	  'name', tsinger.name,
	  'problematic', tsinger.problematic,
	  'aliases', tsinger.aliases,
	  'i18n', tsinger.i18n,
	  'priority', tsinger.priority,
	  'karafile_tag', tsinger.karafile_tag
	)::jsonb) END as singers,
  CASE WHEN MIN(tsongtype.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tsongtype.pk_tid,
	  'short', tsongtype.short,
	  'name', tsongtype.name,
	  'problematic', tsongtype.problematic,
	  'aliases', tsongtype.aliases,
	  'i18n', tsongtype.i18n,
	  'priority', tsongtype.priority,
	  'karafile_tag', tsongtype.karafile_tag
	)::jsonb) END as songtypes,
  CASE WHEN MIN(tsongwriter.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tsongwriter.pk_tid,
	  'short', tsongwriter.short,
	  'name', tsongwriter.name,
	  'problematic', tsongwriter.problematic,
	  'aliases', tsongwriter.aliases,
	  'i18n', tsongwriter.i18n,
	  'priority', tsongwriter.priority,
	  'karafile_tag', tsongwriter.karafile_tag
	)::jsonb) END as songwriters,
  CASE WHEN MIN(tversion.pk_tid::text) IS NULL THEN null ELSE jsonb_agg(DISTINCT json_build_object(
	  'tid', tversion.pk_tid,
	  'short', tversion.short,
	  'name', tversion.name,
	  'problematic', tversion.problematic,
	  'aliases', tversion.aliases,
	  'i18n', tversion.i18n,
	  'priority', tversion.priority,
	  'karafile_tag', tversion.karafile_tag
	)::jsonb) END as versions,
