--We remove kara because we can't re-add title at the same place it was before. You can't move columns in PGSQL
DROP TABLE public.kara;

CREATE TABLE IF NOT EXISTS public.kara
(
    pk_kid uuid NOT NULL,
    title character varying COLLATE pg_catalog."default" NOT NULL,
    year smallint,
    songorder smallint,
    mediafile character varying COLLATE pg_catalog."default" NOT NULL,
    subfile character varying COLLATE pg_catalog."default",
    karafile character varying COLLATE pg_catalog."default" NOT NULL,
    duration smallint DEFAULT 0,
    mediasize integer DEFAULT 0,
    gain real DEFAULT 0,
    created_at timestamp with time zone NOT NULL,
    modified_at timestamp with time zone NOT NULL,
    repository character varying COLLATE pg_catalog."default" NOT NULL,
    title_search_vector tsvector,
    loudnorm character varying COLLATE pg_catalog."default",
    download_status character varying COLLATE pg_catalog."default" DEFAULT 'MISSING'::character varying,
    comment text COLLATE pg_catalog."default",
    CONSTRAINT kara_pkey PRIMARY KEY (pk_kid)
)

TABLESPACE pg_default;

ALTER TABLE public.kara
    OWNER to karaokemugen_app;
-- Index: idx_kara_created

-- DROP INDEX public.idx_kara_created;

CREATE INDEX idx_kara_created
    ON public.kara USING btree
    (created_at DESC NULLS FIRST)
    TABLESPACE pg_default;
-- Index: idx_kara_search_vector

-- DROP INDEX public.idx_kara_search_vector;

CREATE INDEX idx_kara_search_vector
    ON public.kara USING gin
    (title_search_vector)
    TABLESPACE pg_default;
-- Index: idx_kara_songorder

-- DROP INDEX public.idx_kara_songorder;

CREATE INDEX idx_kara_songorder
    ON public.kara USING btree
    (songorder ASC NULLS LAST)
    TABLESPACE pg_default;
-- Index: idx_kara_title

-- DROP INDEX public.idx_kara_title;

CREATE INDEX idx_kara_title
    ON public.kara USING btree
